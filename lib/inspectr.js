...
function capture(req, res, next, options = { broadcast: true, print: true }) {
  ...
  return parseRequestBody(contentType, req, res, {})
    ...
    .then((respPayload) => {
      ...
      // data.timestamp = requestMeta.timestamp; // Use the same timestamp as the request arrival.
...
function print(data) {
  console.log(
    `${data.response.statusCode} - ${data.method} ${data.path} (${data.latency}ms) - ${data.timestamp}`
  );
  return data;
}
